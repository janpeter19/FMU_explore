...
class AdaptWith:
    ...
    def model_get(self, parLoc):
        ...
        for k in range(len(par_var)):
            if par_var[k].name == parLoc:
                try:
                    if (par_var[k].causality in ["local"]) & (
                        par_var[k].variability in ["constant"]
                    ):
                ...
                if name in Location:
                    if not isinstance(self.model_get(Location), np.bool_):
                        print(
                            ...
                            ":",
                            np.round(self.model_get(Location), decimals),
                        )
                    else:
                        print(
                            dict_reverser(parLocation)[Location],
                            ":",
                            self.model_get(Location),
                        )
                else:
                    k = k + 1
                    ...
                    if name in parName:
                        if not isinstance(self.model_get(Location), np.bool_):
                            print(
                                ...
                                ":",
                                np.round(
                                    self.model_get(parLocation[parName]), decimals
                                ),
                            )
                        else:
    ...
    def simu(self, simulationTime=None, mode="Initial", options=None):
        ...
        def extract_variables(diagrams):
            """Extract variables that should be logged during simulation. A help function. """
        ...
        if mode in ["Initial", "initial", "init"]:
            ...
            sim_res = simulate_fmu(
                ...
                output=list(
                    set(
                        extract_variables(diagrams)
                        + list(stateValue.keys())
                        + keyVariables
                    )
                ),
            )
            ...
            if self.prevFinalTime == 0:
                ...
                parLocationMod = dict(
                    list(parLocationRed.items()) + list(stateValueInitialLoc.items())
                )

                # Create parValueMod and parLocationMod
                parValueMod = dict(
                    list(parValueRed.items())
                    + [
                        (stateValueInitial[key], stateValue[key])
                        for key in stateValue.keys()
                    ]
                )

                start_values = {
                    parLocationMod[k]: parValueMod[k] for k in parValueMod.keys()
                }
        ...
        if simulationDone:
            ...
            for key in list(stateValue.keys()):
                stateValue[key] = self.model_get(key)
